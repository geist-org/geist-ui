...
If we don't do anything, the volume of `cfxjs/react-ui` is about `58kb`, note that **the volume here includes lib `styled-jsx`**.
...
The volume of `cfxjs/react-ui` is about `13kb`.(It consists of two parts)
...
If you don't use `tree shaking` in your porject, bundle `cfxjs/react-ui` as a `chunk`
...
- If you want to customize config of webpack, to ensure that the `chunk` from `cfxjs/react-ui` is always the same,
