...
<Note type="warning">Scaleable requires Geist UI not less than version v2.2.0.</Note>

The scalable nature of the Geist UI gives great flexibility to each component.
...
Different from common component libraries, Geist UI no longer uses `size` related props to control component volume.
Each component of Geist UI has a suitable layout volume by default, you can dynamically scale components with the `scale` props.
...
Geist UI renders the specified volume components **realistically** by dynamically calculating the component size.
...
The default scaling unit is `16px`, but Geist UI allows you to customize this value at different levels to achieve a high level of customization.
