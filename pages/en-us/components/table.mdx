...
import { Table, Spacer, Tooltip } from 'components'
  ...
  () => {
    ...
    return (
      <Table variant='line' data={data}>
        ...
        <Column title="Operations" dataIndex="" key="d" render={() => <a href="javascript:void(0)">Operations</a>} />
      </Table>
    )
  }
`}
...
<Playground
  ...
  desc="Display formatted data in rows and columns."
  scope={{ Table, Spacer, useState, AllHTMLAttributes }}
  code={`
  () => {
    function useCheckbox(defaultChecked) {
