...
() => {
  const operation = (actions, rowData) => {
    return <Button type="error" auto scale={1/3} onClick={() => actions.remove()}>Remove</Button>
  }
  const data = [
...
// operation: TableOperation
  const operation = (actions, rowData) => {
    return <Button type="error" auto scale={1/3} onClick={() => actions.update({ property: 'updated', description:'updated' })}>Update</Button>
  }
  const data = [
    { property: 'type', description: 'Content type', operation },
    { property: 'Component', description: 'DOM element to use', operation },
