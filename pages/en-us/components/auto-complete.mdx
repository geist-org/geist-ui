...
() => {
  ...
  <div style={{display: 'flex'}}>
    ...
    <Spacer inline />
    <AutoComplete solid placeholder="Enter here" options={options} />
  </div>)
  }
`}
/>
...
() => {
  ...
  <div style={{display: 'flex'}}>
    ...
    <Spacer inline />
    <AutoComplete solid disabled options={options} initialValue="London" />
  </div>)
}
`}
/>
...
() => {
  ...
  <Spacer inline />
  <AutoComplete solid options={solidOptions} placeholder="Enter here" onSearch={solidSearchHandler} />
  </div>)
  }
`}
/>
...
<div style={{display: 'flex'}}>
  ...
  <Spacer inline />
  <AutoComplete solid searching placeholder="Enter here" width="100%">
    <AutoComplete.Searching>
      <span style={{ color: 'red' }}>waiting...</span>
    </AutoComplete.Searching>
  </AutoComplete>
...
() => {
  ...
  const solidMakeOption = (label, value) => (
    <AutoComplete.Option value={value}>
      <Grid.Container style={{ padding: '10pt 0' }}>
        <Grid xs={24}><Text span b size="1.2rem">Recent search results </Text></Grid>
        <Grid.Container xs={24}>
  ...
  const solidSearchHandler = (currentValue) => {
    if (!currentValue) return solidSetOptions([])
    const relatedOptions = allOptions.filter(item => item.value.includes(currentValue))
    const customOptions = relatedOptions.map(({ label, value }) => solidMakeOption(label, value))
...
() => {
  ...
  <Spacer inline />
  <AutoComplete solid placeholder="Enter here" clearable options={options} />
  </div>)
  }
`}
/>
...
() => {
  ...
  return <AutoComplete options={options} clearable disableFreeSolo placeholder="Enter here" onSearch={searchHandler} onSelect={console.log} />
}
`}
/>
