...
<>
  ...
  <Divider volume={2} y={3} color="success">HTTP</Divider>
  <Text>
    The server uses them as hints and an internal algorithm chooses the best content to serve to the client.
    The algorithm is server-specific and not defined in the standard.
  </Text>
  <Divider volume={20} y={3} color="warning">standard headers</Divider>
  <Text>
    The HTTP/1.1 standard defines list of the standard headers that start server-driven negotiation
    (Accept, Accept-Charset, Accept-Encoding, Accept-Language)
  </Text>
...
| **color**  | color of divider  | [DividerColors](#dividercolors) | -                                | `default` |
...
<Attributes.Title>DividerColors</Attributes.Title>

```ts
type DividerColors = 'default' | 'secondary' | 'success' | 'warning' | 'error' | 'dark' | 'lite'
```
