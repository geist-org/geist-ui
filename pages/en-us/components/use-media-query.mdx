...
If you just want to build the layout, try <NextLink href="/en-us/components/grid"><Link>Grid Component</Link></NextLink>.

This is custom React Hooks, you need to follow the <Link target="_blank" href="https://reactjs.org/docs/hooks-rules.html">Basic Rules</Link> when you use it.
...
<PlaygroundTitle
  ...
  desc="Override the default breakpoints of the `@cfxjs/react-ui`."
/>

```tsx
const breakpoints: CfxUIThemesBreakpoints = {
  xs: { min: '0', max: '650px' },
...
const App = () => (
  <CfxProvider theme={{ breakpoints: breakpoints }}>
    <CssBaseline />
    <AppComponent />
  </CfxProvider>
)
