...
import { Textarea, Spacer, useImperativeInput, Button, Code } from 'components'
...
<Playground
  title={
    <>
      与&nbsp;<Code>useImperativeInput</Code>&nbsp;组合
    </>
  }
  desc="通过 `hooks` 以更简单的方式驱动文本框。"
  scope={{ Textarea, useImperativeInput, Button, Spacer }}
  code={`
() => {
  const { ref, setValue } = useImperativeInput()
  return (
    <>
