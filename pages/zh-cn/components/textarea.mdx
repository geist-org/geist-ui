...
<>
  ...
  <Spacer inline />
  <Textarea solid placeholder="请输入一段描述。" />
</>
`}
/>
...
<>
  ...
  <Spacer inline />
  <Textarea solid width="100%" disabled placeholder="CSS（层叠样式表）用于设置和布置网页 - 例如，更改内容的字体，颜色，大小和间距，将其拆分为多个列，或添加动画和其他装饰功能。" />
</>
`}
/>
...
<Playground
  ...
  code={`
<>
  <Textarea counter maxLength={200} placeholder="限制最多 200 个字符" />
  <Spacer inline />
...
<Playground
  ...
  code={`
<>
  <Textarea status="primary" minHeight="65px" value="主要的" />
  <Spacer x={.5} inline />
  <Textarea status="error" minHeight="65px" value="错误" />
  <Spacer y={.5} />
  <Textarea solid status="primary" minHeight="65px" value="主要的" />
  <Spacer x={.5} inline />
  <Textarea solid status="error" minHeight="65px" value="错误" />
</>
`}
/>
