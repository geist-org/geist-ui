...
<Playground
  ...
  scope={{ Code }}
  code={`
<p>Run <Code>npm i @cfxjs/react-ui</Code> to install.</p>
`}
/>
...
<Playground
  ...
  code={`
() => {
  const codes = \`npm i @cfxjs/react-ui
...
<Playground
  ...
  code={`
() => {
  const codes = \`npm i @cfxjs/react-ui npm i @cfxjs/react-ui npm i @cfxjs/react-ui
yarn add @cfxjs/react-ui\`
  return <Code block width="50%">{codes}</Code>
}
`}
