...
() => {
  ...
  return (
    <>
      <DatePicker locale="zh_CN" onChange={value => console.log(value)} ref={ref} showTime />
      <Spacer y={0.5} />
      <Button auto type="secondary" size="small" onClick={setChange}>
        设置值并聚焦
      </Button>
      <Divider />
      <RangePicker locale="zh_CN" onChange={value => console.log(value)} ref={ref2} showTime />
      <Spacer y={0.5} />
...
| **blur()**     | 移除焦点                                | -                      |
| **focus()**    | 获取焦点                                | -                      |
| **setValue()** | 设置默认值（参考 `defaultValue` props） | `moment` \| `moment[]` |
