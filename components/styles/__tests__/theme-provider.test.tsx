...
import { CfxProvider, CfxUIThemes, Text } from 'components'
...
describe('ThemeProvider', () => {
  ...
  it('should merge themes with custom function', () => {
    const customFunc = (): DeepPartial<CfxUIThemes> => {
      return {
        ...DefaultThemes,
        palette: {
          success: '#ccc',
  ...
  it('should warning when using the wrong merge function', () => {
    ...
    const customFunc = () => 0 as DeepPartial<CfxUIThemes>
    const wrapper = mount(
      <CfxProvider theme={customFunc}>
        <p>test</p>
      </CfxProvider>,
